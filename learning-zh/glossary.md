...
| row vector            | 行向量             | |
| column vector         | 列向量             | |
| ket                   | 右矢／ket          | 形如 $|a\rangle$ 的列向量，正文中通常保留 ket |
| bra                   | 左矢／bra          | 形如 $\langle a|$ 的行向量；复向量中是 ket 的共轭转置 |
| inner product         | 内积               | bra 与 ket 的乘积，结果为标量 |
| outer product         | 外积               | ket 与 bra 的乘积，结果为矩阵 |
| norm                  | 范数               | |
...
| basis                 | 基                 | 不翻译成“基础” |
| standard basis        | 标准基             | |
| standard basis vector | 标准基向量         | 只有一个分量为 1，其余分量为 0 |
...
| English                  | 统一中文译法       | 说明 |
| ------------------------ | ------------------ | ---- |
| classical state          | 经典状态           | 能够被无歧义识别的经典系统配置 |
| classical state set      | 经典状态集合       | 系统所有可能经典状态组成的有限非空集合 |
| classical system         | 经典系统           | |
| deterministic state      | 确定性状态         | |
| probabilistic state      | 概率状态           | 使用概率分布描述对经典系统状态的知识 |
| probability vector       | 概率向量           | 各分量非负且总和为 1 |
| probability distribution | 概率分布           | |
| deterministic operation  | 确定性操作         | 给定输入后，输出完全确定的操作 |
| probabilistic operation  | 概率操作           | 给定输入后，按照概率分布产生输出的操作 |
| stochastic matrix        | 随机矩阵           | 本课程中指元素非负且每一列之和为 1 的矩阵 |
| stationary distribution  | 平稳分布／不变分布 | 满足 $Mp=p$ 的概率向量 |
...
| basis state           | 基矢态／基向量状态 | 所选基中的某个向量；避免与 ground state 混淆 |
...
- `basis state`：表示所选基中的某个向量，例如 $|0\rangle$、$|1\rangle$，优先译为“基矢态”或“基向量状态”。
- `ground state`：表示哈密顿量能量最低的状态，优先译为“最低能态”。
- 不再把两者都简称为“基态”，以免产生歧义。
...
- deterministic operation：确定性操作；
- probabilistic operation：概率操作；
- quantum operation：量子操作；
...
| 2026-07-20 | 补充 Classical information 中的 Dirac 符号、经典操作、随机矩阵和平稳分布术语；修正行内数学格式与 basis state 译法 |
